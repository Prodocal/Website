<script>
    ...
    import {user, stderr, validateLogin, newUser} from "$lib/ApiJS/data.utils.js";
    ...
    const err = Object.create(stderr);
    let pass2 = "";
    ...
    <div class="card w-96">
        <div class="card-body items-center text-center">
            <div class="bg-white rounded-full px-5 py-.5 text-center mt-2 shadow-xl">
                <div class="tabs tabs-bordered">
                    {#each tabs as tab, index}
                        <button class="tab text-[#FFAB9F] text-opacity-50 text-m" class:tab-active={activeTab == index} on:click={()=>activeTab = index} on:click={clearObj(obj)} on:click={clearErr(err)}>{tab}</button>
                    {/each}
                </div>
            </div>
            ...
            <div class="card-body" class:hidden={activeTab != 1}>
                <form class="form-control w-full max-w-xs" on:submit|preventDefault={newUser(obj)} on:submit|preventDefault={validateForm(obj, err, pass2)}>
                    <div class="card-actions justify-start mb-2">
                        ...
                        <br><span id = "nameMsg" style="color:red">{err.name_err}</span>
                    </div>
                    <div class="card-actions justify-start mb-2">
                        ...
                        <br><span id = "emailMsg" style="color:red">{err.em_err}</span>
                    </div>
                    ...
                    <div class="card-actions justify-start">
                        <input type="text" placeholder="username" class="bg-white input w-full text-[#31302e] max-w-xs rounded-full mb-2 shadow-xl placeholder-[#FFAB9F] pl-11" bind:value={obj.username}/>
                        <img src={UsernameIcon} class="absolute w-8 ml-2 mt-2" alt="?">
                        <br><span id = "unameMsg" style="color:red">{err.uname_err} </span>
                    </div>

                    <div class="card-actions justify-start">
                        <input type="password" placeholder="password" class="bg-white input w-full text-[#31302e] max-w-xs rounded-full mb-2 shadow-xl placeholder-[#FFAB9F] pl-11" {...$$restProps} on:input={onInput} bind:value={obj.password}/>
                        <img src={PasswordIcon} class="absolute w-8 ml-2 mt-2" alt="?">
                        <button type="button" on:click="{ () => show_password = !show_password }">{show_password ? 'Show' : 'Hide'}</button>
                        <br><span id = "passwordMsg" style="color:red">{err.pw_err} </span>
                    </div>

                    <div class="card-actions justify-start">
                        <input type="password" placeholder="re-enter password" class="bg-white input w-full text-[#31302e] max-w-xs rounded-full mb-2 shadow-xl placeholder-[#FFAB9F] pl-11" bind:value={pass2}/>
                        <img src={PasswordIcon} class="absolute w-8 ml-2 mt-2" alt="?">
                        <br><span id = "cpasswordMsg" style="color:red">{err.cpw_err} </span>
                    </div>

                    <div class="card-actions justify-center">
                        <button type="submit" class="text-[#FFAB9F] text-lg bg-white hover:text-[#31302e] rounded-full px-10 py-2.5 text-center mt-8 mr-2 mb-2 shadow-xl">
                            <a href="/calendar" class="font-extrabold" on:click|preventDefault={validateForm(obj, err, pass2)}>Signup</a>
                        </button>
                    </div>
                </form>
            </div>
